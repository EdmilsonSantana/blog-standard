...
export default function NewPost(props) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('New Post Result:', json);
          ...
          <form onSubmit={handleSubmit} className="m-auto w-full max-w-screen-sm bg-slate-100 p-4 rounded-md shadow-xl border border-slate-200 shadow-slate-200">
            <div>
              ...
              <textarea className="resize-none border border-slate-500 w-full block my-2 px-4 py-2 rounded-sm"
                value={topic} onChange={e => setTopic(e.target.value)} maxLength={80} />
            </div>
            <div>
              <label>
                <strong>
              ...
              <textarea className="resize-none border border-slate-500 w-full block my-2 px-4 py-2 rounded-sm"
                value={keywords} onChange={e => setKeywords(e.target.value)} maxLength={80} />
